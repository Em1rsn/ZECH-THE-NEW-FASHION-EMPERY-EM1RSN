...
function t754__showSeparator(rec, cardsCount) {
    if (rec instanceof jQuery) { rec = rec.get(0) }
...
function t754__alignButtons_init(recid) {
    ...
    try {
        t754__alignButtons(recid); if (document.querySelector('.t-records').getAttribute('data-tilda-mode') === 'edit') { setInterval(function () { t754__alignButtons(recid) }, 200) }
        window.addEventListener('resize', t_throttle(function () {
            if (window.noAdaptive && window.isMobile) { return }
            t754__alignButtons(recid)
        }, 200)); if (typeof jQuery !== 'undefined') { $('#rec' + recid).find('.t754').bind('displayChanged', function () { t754__alignButtons(recid) }) } else { var wrapperBlock = rec.querySelector('.t754'); if (wrapperBlock) { wrapperBlock.addEventListener('displayChanged', function () { t754__alignButtons(recid) }) } }
        if (window.isMobile) { window.addEventListener('orientationchange', function () { t754__alignButtons(recid) }) }
    } catch (error) { console.log('buttons-v-align error: ' + error.message) }
}
function t754__alignButtons(recid) {
    var rec = document.getElementById('rec' + recid); if (!rec) return; var wrappers = rec.querySelectorAll('.t754__textwrapper'); var maxHeight = 0; var container = rec.querySelector('.t754__parent'); if (!container) return; var itemsInRow = parseInt(container.getAttribute('data-blocks-per-row'), 10); if (!itemsInRow) { itemsInRow = 3 }
    ...
    var count = 1; var wrappersInRow = []; Array.prototype.forEach.call(wrappers, function (element) {
...
function t754_initPopup(recid) {
    var rec = document.getElementById('rec' + recid); var popup = rec.querySelector('.t-popup'); if (!rec) return; var body = document.body; var popup = rec.querySelector('.t-popup'); var linksProd = rec.querySelectorAll('[href^="#prodpopup"]'); Array.prototype.forEach.call(linksProd, function (link) {
        link.addEventListener('click', clickOnceHandler, !1); function clickOnceHandler(event) { event.preventDefault(); var product = link.closest('.js-product'); var productLid = product.getAttribute('data-product-lid'); t_onFuncLoad('t_sldsInit', function () { t_sldsInit(recid + ' #t754__product-' + productLid) }); link.removeEventListener('click', clickOnceHandler, !1) }
        link.addEventListener('click', function (event) {
            event.preventDefault(); var isT1002Button = event.target.classList.contains('t1002__addBtn') || event.target.parentNode.classList.contains('t1002__addBtn'); if (isT1002Button) return; t754_showPopup(recid); var product = link.closest('.js-product'); var productLid = product.getAttribute('data-product-lid'); var productFull = popup.querySelector('.js-product[data-product-lid="' + productLid + '"]'); productFull.style.display = 'block'; var analitics = link.getAttribute('data-track-popup'); if (analitics) {
                var productName = productFull.querySelector('.js-product-name'); if (productName) {
                    var virtTitle = productName.textContent; if (!virtTitle) { virtTitle = 'prod' + productLid }
            ...
            t754_updateSlider(recid + ' #t754__product-' + productLid); if (window.lazy === 'y' || document.getElementById('allrecords').getAttribute('data-tilda-lazy') === 'yes') { t_onFuncLoad('t_lazyload_update', function () { t_lazyload_update() }) }
        })
    }); if (popup) {
...
function t754_checkUrl(recid) {
    var currentUrl = window.location.href; var tprodIndex = currentUrl.indexOf('#!/tproduct/'); if (tprodIndex === -1) { tprodIndex = currentUrl.indexOf('%23!/tproduct/'); if (tprodIndex === -1) { tprodIndex = currentUrl.indexOf('#%21%2Ftproduct%2F') } }
...
function t754_removeSizeStyles(styleStr) {
    if (styleStr && (styleStr.indexOf('font-size') !== -1 || styleStr.indexOf('padding-top') !== -1 || styleStr.indexOf('padding-bottom') !== -1)) {
        var styleStrSplitted = styleStr.split(';'); styleStr = ''; for (var i = 0; i < styleStrSplitted.length; i++) {
            if (styleStrSplitted[i].indexOf('font-size') !== -1 || styleStrSplitted[i].indexOf('padding-top') !== -1 || styleStrSplitted[i].indexOf('padding-bottom') !== -1) { styleStrSplitted.splice(i, 1); i--; continue }
...
function t754_copyTypography(recid) {
    var rec = document.getElementById('rec' + recid); if (!rec) return; var productTitle = rec.querySelector('.t754__title'); var productDescription = rec.querySelector('.t754__descr'); if (!productTitle || !productDescription) { return }
...
function t686_setHeight(recId) {
    var rec = document.getElementById('rec' + recId); if (!rec) return; var container = rec.querySelector('.t686'); if (!container) return; var rows = rec.querySelectorAll('.t686__row'); var col = rec.querySelector('.t686__col'); var ratio = container.getAttribute('data-tile-ratio'); if (!col) return; var colStyle = getComputedStyle(col, null); var colPaddingLeft = parseInt(colStyle.paddingLeft) || 0; var colPaddingRight = parseInt(colStyle.paddingRight) || 0; var colHeight = (col.clientWidth - (colPaddingLeft + colPaddingRight)) * +ratio; var maxHeight = 0; for (var i = 0; i < rows.length; i++) {
        var row = rows[i]; var tables = row.querySelectorAll('.t686__table'); for (var j = 0; j < tables.length; j++) {
            var table = tables[j]; var textWrap = table.querySelector('.t686__textwrapper'); var textWrapHeight = 0; if (textWrap) { textWrapHeight = textWrap.offsetHeight }
        ...
        if (!!document.documentMode) {
            var bgs = row.querySelectorAll('.t686__bg'); var overlays = row.querySelectorAll('.t686__overlay'); var tableHeight = parseInt(row.querySelector('.t686__table').style.height); for (var j = 0; j < bgs.length; j++) { bgs[j].style.height = tableHeight + 'px' }
